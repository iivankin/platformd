...
Для каждой пары OS/architecture выпускается один executable `platformd`. В него через `go:embed` включены:
...
Каждый payload проверяется по checksum из embedded bundle manifest внутри самого binary. Bundle manifest содержит format version, relative path под `runtime/`, file mode, size и SHA-256 каждого helper/config payload; отдельного bundle version нет. Он уже защищён hash/signature внешнего release manifest §31. Binary копируется и payload извлекается в staging slot, весь slot fsync-ится и публикуется одним atomic rename. Helpers не добавляются в host `PATH`; platformd/libpod используют только absolute paths внутри current release slot.
...
```json
{
  ...
  "toolchain": {
    ...
    "ccByArchitecture": {
      ...
      "arm64": "exact-toolchain-id"
    }
  },
  "goBuild": {
    "cgo": true,
    "tags": ["seccomp", "systemd"]
  },
  "helpers": {
    "crun": {
  ...
  "nativeDependencies": {
    "allowedHostSonamesByArchitecture": {
      ...
      "arm64": ["ld-linux-aarch64.so.1", "libc.so.6", "libm.so.6"]
    },
    "staticClosure": [
      {
        "name": "glib",
...
Все fields обязательны, arrays canonical-sorted, unknown fields отклоняются. `staticClosure` содержит **каждый** direct и transitive non-host native library, фактически попавший хотя бы в один output; приведённые names являются текущим ожидаемым minimum, а CI отклоняет как отсутствующий node, так и лишний linked dependency. Source archive hash и effective canonical build flags обязательны для каждого node. Exact Go dependencies, включая pinned/forked libpod/containers modules, имеют единственный source of truth в `go.mod`/`go.sum` и не дублируются в lock. `build.lock.json` фиксирует только toolchain, non-Go helpers/native dependencies, build flags, supported targets/features и external protocol revisions. Release CI проверяет фактическую сборку против lock, затем генерирует embedded bundle manifest с output hashes и внешний signed release manifest с hash готового platformd. Build lock не хранится в SQLite, не синхронизируется runtime и отдельно не подписывается.

До первого `init` operator проверяет published release signature/checksum. Старый release slot целиком не удаляется, пока он является `current` либо `previous`; binary и helpers никогда не очищаются независимо друг от друга.

### 3.2. Поддерживаемые hosts

Release bundle собирается и тестируется отдельно минимум для:

- Ubuntu 24.04 LTS, `amd64` и `arm64`;
- Debian 13, `amd64` и `arm64`.
...
1. Проверяет root privileges, host compatibility, свободное место, `443` и cgroup v2.
2. Создаёт directories и staging initial release slot, копирует туда `platformd`, извлекает private runtime payload и atomically публикует slot.
3. Создаёт SQLite database и, только если файл отсутствует, единственный random 256-bit local master key в `/etc/platformd/master.key`. Существующий path переиспользуется только если это root-owned regular non-symlink file mode `0600` с ровно 32 bytes; любое отклонение останавливает init без перегенерации.
4. Показывает master key в base64url только в interactive root TTY и спрашивает `Ключ сохранён вне VPS? [y/N]`. До ответа `yes` init не продолжает установку и service не запускается. Ответ отдельно не сохраняется; local root всё равно может прочитать файл напрямую.
5. Через interactive prompt/root-only file descriptor принимает в память console passphrase, admin hostname, Cloudflare Origin certificate/key, Access issuer/team domain и application AUD; secrets и TLS private key через command-line arguments запрещены. Полностью валидирует весь набор, но ещё не публикует partial product rows.
6. Atomically устанавливает `/usr/local/bin/platformd` symlink и один `platformd.service`, выполняет daemon-reload и idempotent enable unit, затем одной SQLite transaction создаёт complete installation configuration со всеми initial settings. Partial installation configuration в SQLite отсутствует.
7. Запускает daemon и проверяет HTTPS locally с нужным SNI.
...
Если complete installation configuration уже существует, обычный повторный `init` не спрашивает initial inputs и не меняет product state, но idempotently восстанавливает/проверяет current symlink и unit, выполняет daemon-reload, enable/start и local HTTPS health check. Это закрывает crash после configuration commit, но до шага 7, без bootstrap marker. Все остальные изменения initialized installation разрешены только через явно названные recovery flags либо UI/API.
...
├── releases/
│   ├── current -> <version>/
│   ├── previous -> <version>/      # до readiness target; rollback только до migration commit
│   └── <version>/
│       ├── platformd
...
Bundled SQLite version pin-ится минимум на 3.51.3 либо официальный branch с WAL-reset fix. Все mutations проходят через один bounded writer task. S3/Registry payload I/O и hashing выполняются вне SQLite transaction; metadata commit короткий. Server задаёт busy timeout, ограничивает очередь/transaction duration и выполняет controlled WAL checkpoints. UI/API/MCP используют одни server-side invariants.
...
| Runtime helpers/config | immutable `runtime/` внутри current release slot |
...
Platformd не управляет Cloudflare DNS, firewall или Access policies и не содержит certificate automation. UI хранит один или несколько encrypted Origin certificates, выбирает exact/wildcard certificate по SNI и до создания hostname проверяет SAN coverage; непокрытый route отклоняется. Delete/replace certificate одной transaction разрешён только если после mutation каждый existing public hostname всё ещё имеет exact/wildcard SAN coverage; иначе mutation отклоняется с перечнем зависимых hostnames. После commit выполняется atomic TLS config reload. Automatic expiry monitoring/renewal отсутствует.
...
- загружает JWKS только с configured team domain и кэширует successful response fixed 6 hours;
...
- проверяет signature, allowlisted asymmetric `alg`, `kid`, exact issuer, expiry/not-before с fixed 60-second clock skew и требует, чтобы string либо array claim `aud` содержал exact configured audience;
...
Private Registry responses всегда получают `Cache-Control: private, no-store` и Cloudflare no-store directive. Для public repository digest-addressed immutable payload может cache-иться; mutable tag response требует revalidation/no-store.
...
- ETag является quoted lowercase SHA-256 полного plaintext object и не обещает AWS MD5/multipart-MD5 semantics;
...
Public authenticated/presigned responses принудительно содержат `Cache-Control: private, no-store` и Cloudflare-specific no-store directive. Optional per-store CORS allowlist поддерживает только configured origins, `GET/HEAD/PUT` и необходимые signed headers; preflight `OPTIONS` не требует SigV4, но проверяет allowlist. Без CORS configuration browser cross-origin fetch/PUT запрещён.

### 21.1. Local encryption

Object payload и incomplete multipart parts всегда encrypted at rest. Для каждого ObjectStore единственный 32-byte key детерминированно выводится как `HKDF-SHA-256(masterKey, salt=storeId, info="platformd/s3/store/v1")`; сам store key нигде не хранится и wrappers отсутствуют. Final object делится на fixed-size chunks; multipart temporary chunks используют тот же size. Каждый final chunk шифруется `XChaCha20-Poly1305` тем же store key с новым CSPRNG-generated random 24-byte nonce и associated data `(formatVersion, storeId, immutablePayloadId, chunkIndex, plaintextSize)`. Multipart chunk использует отдельный associated-data domain `(formatVersion, storeId, uploadId, partNumber, chunkIndex, plaintextSize)`. На диске сохраняются только nonce, ciphertext и authentication tag; plaintext temporary parts запрещены, corruption обнаруживается до выдачи plaintext. Вероятностная уникальность 192-bit nonce принимается, отдельный nonce registry не вводится; deterministic nonce запрещён.
...
Embedded/repository database catalog, default versions, supported-entry IDs и runtime catalog sync отсутствуют. Managed profiles принимают только готовые official images из compile-time allowlist:
...
PostgreSQL, Redis, Registry и каждый ObjectStore имеют независимую BackupPolicy с тремя user-facing fields: `enabled` default `false`, `cron` и `retentionCount`. `retentionCount` является integer `1..100`; zero/negative/out-of-range отклоняются, поэтому successful newly published generation никогда не удаляет сама себя. `cron` использует только пятичастный Unix format `minute hour day-of-month month day-of-week` и всегда вычисляется в UTC. Разрешены numeric values, `*`, lists, ranges и steps; weekday `0` означает Sunday. Если day-of-month и day-of-week оба ограничены, occurrence наступает при совпадении любого из них по Vixie cron semantics. Seconds, names, `@daily`-подобные macros, `TZ=` и per-policy/installation timezone отсутствуют. `Backup now`, generation list, retention и restore относятся к одному exact resource.
...
Отдельного recovery key нет. Единственный master key является installation root key. Для каждого resource backup key детерминированно выводится как `HKDF-SHA-256(masterKey, salt=resourceId, info="platformd/backup/v1")`; wrappers, key epochs и online key rotation отсутствуют. Каждый backup chunk шифруется `XChaCha20-Poly1305` с новым CSPRNG-generated random 24-byte nonce и associated data `(formatVersion, resourceId, generationId, chunkIndex, plaintextSize)`. PostgreSQL `pg_dump` stream и копия Redis RDB шифруются до записи в `backups/work`, поэтому plaintext backup file там не сохраняется. S3 chunks уже encrypted at rest по §21.1 и копируются byte-for-byte; S3 metadata шифруется derived backup key.
...
Ordinary application volumes не backup-ятся и их прежние data невосстановимы. После full restore platformd автоматически создаёт empty directory для каждого восстановленного Volume ID; отдельные `payload_missing`, attach-external-data и `Recreate empty volume` states/actions отсутствуют. После выхода из recovery mode enabled services с такими mounts запускаются обычным reconcile без дополнительного confirmation.
...
- `low`: warning/audit transition, отключение opportunistic retention неиспользуемых runtime images, cleanup expired Registry/S3 uploads, temporary files, logs сверх retention/budget, unreferenced runtime images и orphan ObjectStore payloads, если соответствующий store не занят backup/restore; pulls, необходимые для desired deployment, ещё разрешены до перехода в `critical`;
...
7. Только после successful cleanup target выполняет одну bounded transactional SQLite migration. До commit database остаётся совместимой с previous release; commit является необратимой update boundary, после которой rollback previous binary запрещён. Затем target открывает control-plane listeners/admin UI, удаляет `previous` symlink и old release directory; crash между readiness и cleanup оставляет только orphan release artifacts, которые current binary удаляет при следующем successful startup. Для каждого service/DB exact digest сначала ищется в preserved cache; только отсутствующий digest pull-ится из registry. Затем networks/containers reconciled из SQLite. Digest, отсутствующий одновременно в cache и registry, оставляет только соответствующий resource stopped с retry/error в UI и не запускает systemd restart loop.
...
До commit SQLite schema migration local root при остановленном unit может запустить сохранённый binary напрямую: `/var/lib/platformd/releases/previous/platformd init --rollback-update`. Команда atomically возвращает `current` на previous slot и только затем запускает unit; она не зависит от того, способен ли текущий symlink target стартовать. Previous binary также проходит clean-runtime startup и пытается открыть текущий graphroot. Если open fails, cache удаляется и images pull-ятся заново. Automatic rollback supervisor не вводится.

После successful SQLite commit обычный rollback запрещён. Если target не достигает readiness, сохранённый previous binary остаётся доступен **только как bootstrap verifier/installer**: local root останавливает unit и запускает `/var/lib/platformd/releases/previous/platformd init --install-signed-update <manifest> [--binary <path>]`, чтобы установить strictly newer signed forward fix без открытия уже migrated SQLite. Previous daemon/runtime на новой schema не запускается. Если forward fix недоступен, остаётся full restore из remote control/resource backups; сложная post-commit rollback state machine отсутствует. Local previous binary автоматически удаляется только после readiness и не является долгосрочным recovery artifact.
...
- update tests подтверждают idle-only admission, `409 platform_busy` без cancellation, non-blocking reads/log streams, запрет новых mutations после gate, old-process graceful stop без duplicate teardown, cleanup строго до migration, rollback direct previous binary до commit, signed forward install previous binary после commit без открытия SQLite, полный workload downtime, recreation runtime dir, reuse populated image cache после container-record purge, incompatible-cache fallback, missing image digest, crash до/после atomic `current` switch и migration commit, отсутствие pre-update SQLite backup и automatic deletion previous symlink/slot только после readiness;
- release-manifest tests используют RFC 8785 canonical bytes, отклоняют malformed/duplicate JSON keys, signature/hash/size/OS/architecture/`supportedFrom` mismatch, reinstall/downgrade и подтверждают automatic cache wipe без compatibility metadata;
...
- managed-image selection tests подтверждают stateless paginated official tag listing, manual tag fallback, per-architecture digest resolution, отсутствие database catalog/defaults, pin exact digest и explicit failure вместо floating fallback при missing digest; engine profile tests на representative pre-18/18+ PostgreSQL и Redis tags проверяют fixed mount/config paths, hidden PostgreSQL bootstrap superuser, non-superuser owner auth/`SELECT 1`, Redis config readability после privilege drop и `AUTH`/`PING`;
...
| ID | Решение |
|---|---|
| V2-01 | Single-file self-extracting distribution; binary и private runtime helpers публикуются вместе в одном self-contained release slot |
...
| V2-31 | S3 использует один HKDF-derived key на ObjectStore и XChaCha20-Poly1305 chunks; wrappers и online key rotation отсутствуют |
...
| V2-33 | Все non-S3 backup keys HKDF-derived из master key; wrappers/key epochs/rotation отсутствуют |
| V2-34 | `init --restore` импортирует latest control generation до запуска admin UI, затем независимо latest complete generation каждого resource; отсутствующий backup и ordinary application volumes автоматически создаются empty без missing-payload state/confirmation |
...
| V2-41 | External release manifest минимален и signed; binary проверяется по size/SHA-256, helper hashes находятся только внутри binary |
...
| V2-47 | Один `build.lock.json` фиксирует per-architecture toolchains, helper flags и полный hashed static native closure; Go deps остаются только в go.mod/go.sum, output hashes — в embedded bundle manifest |
...
| V2-49 | MCP использует stateless JSON-response subset Streamable HTTP с exact 2025-11-25 initialize/serverInfo/header/Accept contracts, `GET=405`, без session ID, SSE, server notifications и resumability |
...
| V2-65 | Private glibc/ELF loader/shared libraries не поставляются; release использует host glibc baseline, а полный hashed non-host closure статически link-ится и проверяется против build lock |
