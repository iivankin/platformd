...
Продукт распространяется одним executable-файлом `platformd`. Он содержит control plane, HTTPS proxy, admin UI, OCI Registry, private S3-compatible store и приватный container-runtime bundle. Базовые shared libraries поддерживаемой host OS используются динамически по контракту §3.1.
...
`platformd init` публикует целостный release slot, в котором binary и соответствующий ему runtime payload всегда находятся вместе:
...
Bundle не содержит private glibc/ELF-loader/shared-library tree. Platformd, crun, conmon и Netavark динамически используют только зафиксированные SONAME из базового ABI поддерживаемых Ubuntu/Debian: loader/glibc, libm, GCC unwind runtime, libseccomp, libcap, json-c, GLib, PCRE2 и libatomic. Эти packages не копируются в release slot и обновляются обычными security updates host OS. Единственное exact static output — `catatonit`: он bind-mount-ится внутрь arbitrary glibc/musl/scratch OCI rootfs как PID 1 и поэтому не может зависеть от loader/libraries image. `platformd init` не запускает package manager: отсутствие либо несовместимость любой required host library является failed host probe до установки. Libseccomp обязателен для OCI seccomp policy, а GLib остаётся upstream dependency conmon; их самостоятельные fork/rewrite не вводятся.

Release CI собирает Linux/amd64 artifact один раз против Ubuntu 24.04 baseline, затем проверяет **те же bytes** на чистых Ubuntu 24.04 и Debian 13. На каждом host `readelf`/`ldd` проверяют полный direct/transitive dependency graph против exact SONAME allowlist из build lock и требуют отсутствие ELF interpreter только у exact static-output allowlist; bundled loader/shared-library tree и любой незаявленный SONAME/static output запрещены. Отдельный executable-dependency probe проверяет, что runtime не вызывает незаявленные host commands. Затем CI выполняет реальный create/start/exec/network/remove container cycle минимум с glibc и musl application images, подтверждая запуск static `catatonit` внутри обоих rootfs. Случайная зависимость от host Podman, GPGME, CNI, hooks, CDI, SELinux/AppArmor files, `/etc/containers` или других host runtime tools запрещена.
...
```json
{
  ...
  "features": {
    "cgroupV2": true,
    "deviceControllerBpf": true,
    "netavarkFirewall": false,
    "nftablesNetlink": true,
    "seccomp": true,
    "overlayfs": true,
    "nftables": true,
    "systemdScopes": false
...
Все fields обязательны, arrays canonical-sorted, unknown fields отклоняются. `goBuild.tags` намеренно оставляет libpod на `cgroupfs`, file events и `k8s-file`: tag `systemd` запрещён, потому что platformd не использует libpod journald/systemd-cgroup integration и не должен получать dependency на libsystemd. AppArmor, libsubid, Btrfs и ZFS graph drivers также не входят в runtime profile; OpenPGP implementation не зависит от host GPGME. `deviceControllerBpf=true` означает встроенную в `crun` загрузку минимальной cgroup-v2 eBPF device policy и не добавляет dependency на `libbpf` либо host command; сборка `crun` с `--disable-bpf` запрещена. `allowedHostSonamesByArchitecture` содержит полный union direct/transitive ELF dependencies всех dynamic outputs, а `staticOutputsByArchitecture` — единственные outputs без ELF interpreter; CI отклоняет missing/extra SONAME либо unexpected static/dynamic linkage и выполняет helpers на обоих hosts. Source archive hash и effective canonical build flags обязательны для каждого helper. Exact Go dependencies, включая pinned/forked libpod/containers modules, имеют единственный source of truth в `go.mod`/`go.sum`, а frontend dependencies, включая `ghostty-web`, — в `bun.lock`; они не дублируются в top-level lock. `build.lock.json` фиксирует toolchains, non-Go helpers, build flags, host ABI, supported targets/features и external protocol revisions. Release CI проверяет фактическую сборку против lock, затем генерирует bundle manifest с output hashes и внешний signed release manifest с hash готового platformd. Build lock не хранится в SQLite, не синхронизируется runtime и отдельно не подписывается.

Первый production `init` сам проверяет published release manifest/signature/checksum по §5. Старый release slot целиком не удаляется, пока он является `current` либо `previous`; binary, manifest и helpers никогда не очищаются независимо друг от друга.
...
Libpod использует cgroupfs manager только внутри delegated `platformd.service` subtree. Unit задаёт `DelegateSubgroup=control`: systemd сразу помещает main platformd process в leaf `control/`, а пустой unit cgroup остаётся inner node, где можно включить cgroup v2 controllers. Platformd создаёт sibling subtrees `workloads/` и `operations/`; каждый container payload/catatonit получает собственный leaf под `workloads/`, host PTY/root exec — ephemeral leaf под `operations/`, а conmon остаётся в `control/`, чтобы участвовать в graceful shutdown. Emergency freeze применяется только к `workloads/`, поэтому root cleanup surfaces остаются рабочими. External systemd scopes отсутствуют.
...
На каждый project создаётся отдельная IPv4-only rootful bridge network через libpod/Netavark с network DNS disabled и IPv6 explicitly disabled. Netavark работает только как bridge/veth/IPAM backend; его firewall driver жёстко `none`. Containers разных projects не подключаются к одной project network. Platformd не хранит назначенный subnet/gateway как второй authoritative state: на каждом startup он сортирует Projects по stable ID, сканирует current host routes/interfaces и deterministic выбирает свободный `/24` из compile-time pool `10.80.0.0/12`, затем получает actual parameters через libpod/Netavark inspect. Default route не считается collision, потому что она не резервирует адресный prefix; остальные пересекающиеся IPv4 routes и interface subnets пропускаются. Collision либо исчерпание pool останавливает только affected project workloads с visible error; admin control plane остаётся доступен.

Platformd реализует небольшой DNS server и для каждой project network слушает UDP и TCP `53` на inspected bridge gateway IP. Netavark может создать сам bridge только при setup первого container, поэтому gateway listeners создаются заранее с Linux `IP_FREEBIND`; это не требует dummy container либо отдельного persistent network state. После появления bridge address тот же socket начинает принимать traffic. Каждый project container получает этот IP как единственный resolver. Service получает записи:
...
Bind к разным local gateway IP сам по себе не является isolation boundary из-за Linux weak-host model. Platformd является единственным владельцем container firewall state и напрямую через nf_tables netlink atomically заменяет одну private table `inet platformd`; executable `nft`, `iptables`, firewalld и Netavark firewall backend не используются.

Таблица содержит:

- targeted `input` chain с fixed priority `-150` (после conntrack, до ordinary filter): сначала разрешает `established,related` ответы на host-initiated proxy/health/runtime connections, затем с exact project bridge допускает новые connections только к DNS `53` и internal S3 `9000` на gateway того же project; доступ к этим listeners с host/public/других project interfaces и новые connections project containers к остальным host endpoints отклоняются;
- `forward` chain с fixed priority `-150`: разрешает established/related return traffic, east-west внутри одного project и исходящий traffic project → non-project interface, но отклоняет forwarding между project bridges;
- IPv4 `postrouting` NAT chain с standard `srcnat` priority: masquerade только для source subnet соответствующего project при выходе через non-project interface.

Rules используют inspected bridge names/subnets/gateways, публикуются вместе с pre-bound listeners до запуска containers и удаляются после их остановки. Table/rules не являются authoritative state и полностью пересоздаются из current networks; external deletion обнаруживается reconcile и восстанавливается. До workloads platformd напрямую устанавливает и проверяет `net.ipv4.ip_forward=1`; shutdown не возвращает прежнее global значение, потому что это dedicated single-purpose host prerequisite. Platformd не меняет provider/host firewall rules вне собственной table. `init` active probe применяет и удаляет temporary nf_tables transaction, а полная packet semantics проверяется release acceptance на clean supported hosts; конфликтующая later host policy может сознательно заблокировать traffic и считается unsupported host configuration. Exact locked Netavark release проходит packet-level tests с `firewall_driver=none`, пустым runtime `PATH` без `nft`/`iptables` и единственной platform-owned table.
...
Все services являются long-running processes. User-defined cron/one-shot modes отсутствуют. Отдельных watch/restart/mode settings нет: tag reference автоматически poll-ится по §10, digest reference pinned и не poll-ится, а любой enabled service после unexpected process exit перезапускается platformd с fixed in-memory crash-loop delays `1s → 2s → 4s → 8s → 16s → 30s`, затем остаётся на 30-second cap. После 5 minutes непрерывной работы attempt counter сбрасывается; daemon restart также сбрасывает его. Во время остановленного control plane restart guarantee отсутствует.
...
Platformd использует manifest polling только для Services с remote tag reference: normal interval 60 seconds, а при registry/pull errors in-memory exponential backoff ограничен 15 minutes. Service с digest reference не poll-ится. Embedded registry не poll-ится: после successful manifest/tag SQLite commit тот же process вызывает coalescing in-memory reconcile callback для Services, которые ссылаются на изменённый repository/tag. Durable outbox и отдельный notification state отсутствуют. Если daemon падает после commit до callback, ordinary startup reconcile обнаруживает новый digest. Transient failure callback-triggered embedded reconcile retry-ится с 10-second base и maximum 1 minute; failed candidate той же immutable pair остаётся blocked по общему правилу ниже. Conditional requests применяются к remote registry requests.
...
Admin origin использует строгий CSP без `unsafe-inline`/`unsafe-eval`, output escaping и Trusted Types where supported. Для embedded WASM Ghostty-Web `script-src` явно разрешает только `'self'` и `'wasm-unsafe-eval'`, а его embedded `data:application/wasm` fetch — только через `connect-src 'self' data:`; JavaScript `eval` этим не разрешается. Inline preview разрешён только для bounded plain text и decoded raster images. HTML, SVG, PDF и любой другой active/binary content никогда не исполняется или render-ится в admin origin и доступен только как download. Secrets не помещаются в DOM attributes, URLs или browser storage.
...
Layout использует flat sections, tables, tabs и side panels; card внутри card запрещён. Вложенность выражается spacing, dividers и typography, а не каскадом обрамлённых panels.
...
Full-text search engine и внешний log cluster отсутствуют. Предусмотрены simple contains filter по загруженному bounded окну и NDJSON download выбранного временного диапазона: range больше 24 hours отклоняется, output никогда не превышает 100 MiB и заканчивается explicit completion record с `truncated` flag.

Собственные platformd logs пишутся только в journald и доступны в отдельном Infrastructure view. Platformd читает их bounded subprocess `journalctl --output=json` для собственного systemd unit, без libsystemd/cgo API и без отдельного platformd structured sink/rotation.
...
Interactive server console в admin UI создаёт root PTY на host через exact-pinned `github.com/creack/pty` из `go.mod`; собственная реализация `/dev/ptmx` отсутствует. Она требует одновременно:
...
Platformd выполняет не больше одной Argon2id verification одновременно и не отвечает на failed attempt раньше fixed 2 seconds. После пяти последовательных ошибок действует один global in-memory cooldown 60 seconds; successful verification сбрасывает counter, daemon restart также сбрасывает counter/cooldown. Per-subject/per-IP и persisted passphrase counters отсутствуют. Успешная проверка разрешает открыть terminal session с fixed 30-minute idle timeout и 8-hour absolute lifetime. Host shell запускается как `/bin/bash --login` в отдельном delegated cgroup leaf; закрытие session выполняет `cgroup.kill`, закрывает PTY и удаляет leaf, поэтому обычные foreground/background descendants не переживают disconnect. Содержимое/keystrokes не записываются; audit содержит actor, source IP, timestamps, duration и close reason.

UI отправляет passphrase только в JSON body Access-only `POST /api/v1/server/terminal-token` на admin hostname. Endpoint применяет обычные exact Origin/CSRF checks, выполняет описанную Argon2id verification и при успехе возвращает с `Cache-Control: no-store` короткоживущий HMAC-подписанный Bearer со сроком 30 seconds. Signing key domain-separated от master key; payload содержит только version, exact purpose `server_terminal`, Access subject, issued-at и expiry. Token не сохраняется в SQLite или памяти и не создаёт auth/session/nonce row.

Native browser WebSocket не позволяет задать `Authorization`, поэтому UI открывает `/api/v1/server/terminal` с двумя offered `Sec-WebSocket-Protocol` values: fixed `platformd-terminal-v1` и `platformd-bearer.<base64url-token>`. Platformd извлекает credential из второго value, но при successful upgrade echo-ит только fixed protocol. Query parameter, cookie и localStorage для token запрещены. До upgrade server заново проверяет Access JWT, exact Origin, purpose/signature/expiry Bearer и совпадение Access subject, после чего запускает PTY. Expiry ограничивает только момент открытия: уже открытая PTY живёт по собственным idle/absolute limits. Stateless token не является one-time: его replay в пределах 30 seconds вместе с той же Access identity может открыть ещё одну PTY; этот bounded риск принимается вместо nonce/session state.

### 16.3. Automation root exec

REST endpoint `POST /api/v1/server/exec` и MCP tool `server_exec` выполняют одну non-interactive shell command как host root и возвращают separate bounded stdout, stderr, exit code, timeout/truncation flags и duration. Command ограничен 64 KiB; stdout и stderr независимо ограничены 1 MiB; default timeout 30 seconds, maximum 5 minutes; installation-wide одновременно выполняются максимум 4 команды. Command запускается через fixed `/bin/sh -lc` в отдельном delegated cgroup; при timeout либо request cancellation platformd best-effort убивает все процессы, оставшиеся в этом cgroup subtree. API само не создаёт PTY/background mode, но arbitrary host-root command способен намеренно выйти из cgroup, создать systemd unit либо другой persistent process; отсутствие detached process не является security guarantee. Для interactive работы остаётся UI terminal §16.2.
...
Push всегда требует generated robot credential. Basic username содержит random public credential ID, password — 256-bit secret и используется только на standard Distribution token exchange `/v2/token`; exchange выполняет один indexed SQLite lookup, constant-time HMAC verification и per-ID/source rate limit. Он выдаёт short-lived `5 minute` stateless Bearer token, подписанный HMAC ключом, domain-separated от installation master key; token rows, refresh tokens, sessions и signing-key state отсутствуют. Каждый RegistryCredential принадлежит ровно одному repository и имеет fixed `pull` либо `pull+push` permission; multi-repository grants отсутствуют. Bearer содержит exact repository ID/name и granted actions, а каждый Registry request заново проверяет signature/expiry/scope и существование credential row, поэтому revoke действует немедленно, не ожидая expiry. Каждый upload request дополнительно сверяет upload session repository. Private repository требует credential для pull. Для public repository token endpoint выдаёт anonymous pull-only token без Basic; push token anonymous не бывает. Secret показывается один раз.
...
Manifest PUT имеет hard size/media-type/reference limits, разрешает только blobs, физически существующие в directory того же repository, а OCI index — только child manifests того же repository. Одной SQLite transaction он сохраняет bounded manifest bytes и tag; persistent reachability graph отсутствует. После commit in-process callback coalesces wake-up Services exact repository/tag; callback не является второй commit boundary и не имеет durable outbox.

Fixed v1 Registry quotas не настраиваются и не сохраняются в SQLite: repository name — максимум `255` bytes, tag — максимум `128` bytes, manifest — `4 MiB` и максимум `10 000` descriptor references, `100 000` manifests на repository, `32` одновременно обрабатываемых upload requests на installation, `4` на один credential, `16` incomplete upload sessions на credential и `20 GiB` aggregate temporary upload bytes. Disk-pressure admission §27 может остановить запись раньше. Отдельных user-facing repository storage quotas нет.

Для заявленного subset platformd следует Distribution contract буквально. `GET /v2/` без valid token отвечает `401` с `WWW-Authenticate: Bearer realm="https://<registry-host>/v2/token",service="<registry-host>"`; repository challenge добавляет exact `scope="repository:<name>:pull"` либо `pull,push`. Token endpoint принимает standard `service` и один repository `scope`, возвращает одинаковые `token`/`access_token`, `expires_in=300` и `issued_at`; arbitrary/multi-repository scopes отклоняются. Все responses под `/v2/`, включая token endpoint и `4xx`, возвращают `Docker-Distribution-API-Version: registry/2.0`. Successful blob/manifest `GET` и `HEAD` возвращают `Docker-Content-Digest`, одинаковые `Content-Length`/`Content-Type`, а upload responses — canonical `Location` и требуемый `Range`; tag pagination использует bounded `n`, lexical order и RFC 5988 `Link: rel="next"`. Ошибки имеют соответствующий HTTP status и Distribution JSON error code. Неподдерживаемая операция отклоняется явно, а не маскируется несовместимым успешным ответом.
...
Дальнейший backup копирует blobs только из материализованного списка в новый independent Registry generation prefix. Cleanup exclusion и общий Registry resource mutex остаются до окончания копирования, поэтому перечисленные immutable files не исчезнут: push/tag/manifest mutations уже могут продолжаться, но repository deletion и explicit cleanup получают `409 resource_busy`. Payloads из старых generations не переиспользуются; durable pins и abandoned-pin cleanup не нужны. Completion marker публикуется последним, transient local manifest удаляется, incomplete remote prefix очищается после TTL.
...
Platformd привязывает один S3 listener к inspected gateway IP каждой project bridge, например `10.90.1.1:9000`, тем же pre-bind `IP_FREEBIND` mechanism §8; несколько stores этого project маршрутизируются по exact internal hostname. Listener не bind-ится к `0.0.0.0`. Platform-owned nf_tables policy разрешает его только с соответствующего project bridge. Storage metadata и payload остаются single-writer state platformd; sidecar/gateway container отсутствует.
...
Встроенного release-managed списка версий и default version нет. Managed profiles принимают только готовые official images из compile-time allowlist:
...
SQL console доступна только interactive admin UI через Cloudflare Access. Она выполняет arbitrary SQL, включая multi-statement, transactions, DDL и DML, от generated managed owner role внутри выбранной database; PostgreSQL superuser/cluster-level privileges не выдаются. Platformd не парсит и не классифицирует SQL. Один request принимает до 256 KiB SQL, выполняется максимум 30 seconds, возвращает до 32 statement results, суммарно до 1000 rows и 1 MiB raw cell data; installation-wide одновременно выполняются максимум 4 PostgreSQL UI queries. Достижение result cap помечает output как truncated и server drain-ит protocol response; disconnect/timeout отменяет query через PostgreSQL protocol. Query text/history server-side не сохраняется и в audit не попадает; audit содержит Access actor, database, timestamps, duration, result/row count и SQLSTATE/error class без row values. Public REST, API-token и MCP PostgreSQL data/query endpoints отсутствуют. Позднее для agents может быть добавлен отдельный project-scoped port-forward к native database protocol; в v1 такого surface нет.
...
- privileged integration tests одних и тех же release bytes на каждом supported host, включая Netavark `firewall_driver=none`, отсутствие `nft`/`iptables` в runtime `PATH`, единственную `inet platformd` table и packet-level DNS/S3/input isolation/east-west/cross-project drop/egress masquerade cases;
...
- deployment tests подтверждают stop-first/no-overlap, simplified HTTP/process readiness, restart old при failure, immutable config snapshot, exact crash-loop delays `1/2/4/8/16/30/30…s`, reset после 5 minutes stable runtime, отсутствие restart после planned withdraw, stop/unpublish при disable, fresh Deployment при re-enable и rollback через копирование snapshot с pin exact digest;
...
- managed-image selection tests подтверждают stateless paginated official tag listing, manual tag fallback, host-architecture digest resolution, отсутствие release-managed version list/defaults, pin exact digest и explicit failure вместо floating fallback при missing digest; engine profile tests на representative pre-18/18+ PostgreSQL и Redis tags проверяют fixed mount/config paths, hidden PostgreSQL bootstrap superuser, non-superuser owner auth/`SELECT 1`, Redis config readability после privilege drop и `AUTH`/`PING`;
...
| V2-22 | Release-managed database version list/defaults отсутствуют; UI stateless запрашивает tags official PostgreSQL/Redis repositories и допускает manual tag input |
...
| V2-32 | Remote image watcher poll-ит tag каждые 60 секунд с error backoff до 15 минут; embedded manifest commit вызывает coalesced in-process callback без polling/outbox и с transient retry до 1 минуты; failed candidate pair не повторяется автоматически до нового digest/config либо manual Redeploy |
...
| V2-53 | Service имеет только `enabled` и image reference: tag автоматически poll-ится, digest pinned, а enabled long-running process перезапускается с in-memory delays 1/2/4/8/16/30 секунд и reset после 5 минут stable runtime; watch/restart/mode settings отсутствуют |
...
| V2-65 | Один Linux/amd64 artifact тестируется byte-for-byte на Ubuntu 24.04 и Debian 13; private loader/shared-library tree не поставляется, exact static output только `catatonit`, а полный dynamic host dependency graph проверяется против SONAME allowlist build lock |
...
| V2-72 | Netavark работает с `firewall_driver=none` только как bridge/veth/IPAM backend; platformd напрямую через nf_tables netlink единолично владеет forwarding, masquerade и project isolation в одной recreated table без host `nft`/`iptables` commands |
