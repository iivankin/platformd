...
import { GitHubWorkflowCombobox } from "@/github-workflow-combobox";
...
const ToggleAction = ({
  ...
  <button
    ...
    className="flex min-h-11 w-full items-center gap-3 px-4 text-left hover:bg-muted/40 disabled:cursor-not-allowed disabled:opacity-45"
    disabled={disabled}
...
const workflowInputsError = (value: string): string | undefined => {
  try {
    ...
    if (Object.keys(parsed).length > 25) {
      return "GitHub accepts at most 25 top-level input fields.";
    }
  } catch {
    return "Inputs contain invalid JSON.";
  }
};
...
export const ServiceBeforeDeploy = ({
  ...
  const githubSource = source.type === "github" ? source.github : undefined;
  const inputsError = draft.githubWorkflowEnabled
    ? workflowInputsError(draft.githubInputs)
    : undefined;
    ...
    <SectionCard className="grid lg:grid-cols-[14rem_minmax(18rem,1fr)]">
      <div className="px-5 py-4">
        ...
        <p className="mt-3 font-mono text-[8px] leading-4 text-muted-foreground">
          01 command → 02 workflow → 03 purge
        </p>
      </div>
          ...
          <div className="grid grid-cols-[2.25rem_minmax(0,1fr)]">
            ...
            <div>
              <ToggleAction
                ...
                enabled={draft.githubWorkflowEnabled}
                label="Dispatch and wait for a GitHub workflow"
                onChange={(githubWorkflowEnabled) =>
                  onChange({ ...draft, githubWorkflowEnabled })
                }
              />
              {githubSource ? null : (
                <p className="border-t border-border px-4 py-3 text-[8px] leading-4 text-muted-foreground">
                  Available when the service source is a GitHub repository.
                </p>
              )}
              {draft.githubWorkflowEnabled && githubSource ? (
                <div className="grid gap-3 border-t border-border px-4 py-3">
                  <label
                    className="grid gap-1.5 text-[9px] text-muted-foreground"
                    htmlFor="before-deploy-workflow"
                  >
                    Workflow with workflow_dispatch
                    <GitHubWorkflowCombobox
                      id="before-deploy-workflow"
                      onChange={(githubWorkflow) =>
                        onChange({ ...draft, githubWorkflow })
                      }
                      repositoryID={githubSource.repositoryId}
                      value={draft.githubWorkflow}
                    />
                  </label>
                  <label
                    className="grid gap-1.5 text-[9px] text-muted-foreground"
                    htmlFor="before-deploy-workflow-inputs"
                  >
                    Inputs · JSON object
                    <textarea
                      ...
                      autoCapitalize="none"
                      autoComplete="off"
                      className="min-h-24 resize-y border border-input bg-background px-2.5 py-2 font-mono text-[10px] leading-4 text-foreground outline-none placeholder:text-muted-foreground/55 focus-visible:border-ring focus-visible:ring-1 focus-visible:ring-ring aria-invalid:border-destructive"
                      id="before-deploy-workflow-inputs"
                      onChange={(event) =>
                        onChange({
                          ...draft,
                          githubInputs: event.target.value,
                        })
                      }
                      placeholder={'{\n  "migration": "up"\n}'}
                      spellCheck={false}
                      value={draft.githubInputs}
                    />
                  </label>
                  {inputsError ? (
                    <p className="text-[8px] text-destructive">{inputsError}</p>
                  ) : (
                    <p className="text-[8px] leading-4 text-muted-foreground">
                      Dispatched on {githubSource.branch}; platformd waits up to
                      30 minutes for a successful run.
                    </p>
                  )}
                </div>
              ) : null}
              ...
              {draft.cloudflareEnabled ? (
                <div className="grid gap-2 border-t border-border px-4 py-3">
                  {domains.length ? (
                        ...
                        >
                          <Checkbox
                            ...
                            onCheckedChange={(next) =>
                              onChange({
                                ...draft,
                                cloudflareHostnames: next
                                  ? [
                                      ...draft.cloudflareHostnames,
                                      domain.hostname,
                                    ]
                    ...
                    <p className="text-[8px] leading-4 text-muted-foreground">
                      Add an HTTP domain to select it for cache purge.
                    </p>
                  )}
                </div>
              ) : null}
