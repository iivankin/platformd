...
// WithSignalHandling controls libpod's process-wide SIGINT/SIGTERM handler.
// Embedders that own their process lifecycle must disable it before the
// runtime is finalized; upstream defaults remain unchanged for Podman.
func WithSignalHandling(enabled bool) RuntimeOption {
	return func(rt *Runtime) error {
		if rt.valid {
			return define.ErrRuntimeFinalized
		}
		rt.manageSignals = enabled
		return nil
	}
}

// WithConmonCleanupCommand controls whether conmon invokes the current
// executable with Podman CLI cleanup arguments after a container exits.
// When disabled, the embedding process must Wait and Cleanup containers; a
// crash must be handled by its next-start cleanup of the private graphroot.
func WithConmonCleanupCommand(enabled bool) RuntimeOption {
	return func(rt *Runtime) error {
		if rt.valid {
			return define.ErrRuntimeFinalized
		}
		rt.conmonCleanup = enabled
		return nil
	}
}
...
// WithLogRotation enables conmon's native file rotation and sets the number
// of retained files. Platformd pins a conmon revision that supports
// --log-rotate and --log-max-files.
func WithLogRotation(maxFiles uint) CtrCreateOption {
	return func(ctr *Container) error {
		if ctr.valid {
			return define.ErrRuntimeFinalized
		}
