...
func makeRuntime(ctx context.Context, runtime *Runtime) (retErr error) {
	...
	for name, paths := range runtime.config.Engine.OCIRuntimes {
		ociRuntime, err := newConmonOCIRuntime(name, paths, runtime.conmonPath, runtime.runtimeFlags, runtime.config, runtime.conmonCleanup)
		if err != nil {
			// Don't fatally error.
	...
	if runtime.config.Engine.OCIRuntime != "" {
		...
		if strings.HasPrefix(runtime.config.Engine.OCIRuntime, "/") {
			ociRuntime, err := newConmonOCIRuntime(runtime.config.Engine.OCIRuntime, []string{runtime.config.Engine.OCIRuntime}, runtime.conmonPath, runtime.runtimeFlags, runtime.config, runtime.conmonCleanup)
			if err != nil {
				return err
			}
