...
pub(crate) fn sentry_trace_rows(service_id: &str, payload: &Value) -> Result<Vec<Value>> {
    let trace = payload
        .pointer("/contexts/trace")
        .and_then(Value::as_object)
        .ok_or_else(|| Error::InvalidRequest("Sentry transaction has no trace context".into()))?;
    let trace_id = trace
        .get("trace_id")
        .and_then(Value::as_str)
        .and_then(|value| normalize_hex_id(value, 32))
        .ok_or_else(|| Error::InvalidRequest("Sentry transaction trace ID is invalid".into()))?;
    let received_at = unix_nanos()?;
    ...
    spans.push(sentry_span_row(
        ...
        trace.get("op").and_then(Value::as_str),
...
fn sentry_span_row(
    ...
    let parent_span_id = parent_span_id
        .and_then(|value| normalize_hex_id(value, 16))
        .unwrap_or_default();
    let start = sentry_timestamp_nanos(start)
    ...
    let kind = match operation.unwrap_or_default() {
        value if value.starts_with("http.server") || value.starts_with("server") => 2,
        value
            if value.starts_with("http.client")
                || value.starts_with("db")
                || value.starts_with("rpc.client") =>
        {
            3
        }
        value if value.starts_with("queue.publish") || value.starts_with("messaging.publish") => 4,
        value if value.starts_with("queue.process") || value.starts_with("messaging.process") => 5,
        _ => 1,
    };
    Ok(json!({
        "service_id": service_id,
        ...
        "status_code": status_code,
        "status_message": status.unwrap_or_default(),
        "flags": 0,
        "resource": resource,
        "scope": scope,
        "span": serde_json::to_string(payload).map_err(|error| Error::Storage(format!("encode Sentry span: {error}")))?,
        "received_at_unix_nano": received_at,
        "version": received_at,
        "source": "sentry",
...
fn sentry_timestamp_nanos(value: Option<&Value>) -> Option<u64> {
    let nanos = match value? {
        Value::Number(value) => decimal_seconds_nanos(&value.to_string())?,
        Value::String(value) => Timestamp::from_str(value).ok()?.as_nanosecond(),
        _ => return None,
    };
    u64::try_from(nanos).ok()
}

fn decimal_seconds_nanos(value: &str) -> Option<i128> {
    ...
    let fraction = if fraction.is_empty() {
        ...
        let digits = fraction.parse::<i128>().ok()?;
        digits.checked_mul(10_i128.pow(9_u32.saturating_sub(fraction.len() as u32)))?
    };
...
mod tests {
    ...
    #[test]
    fn sentry_transactions_become_deduplicated_trace_spans() {
        let rows = sentry_trace_rows(
            ...
            &json!({
                ...
                "spans": [{
                    ...
                    "op": "db.sql.query",
                    "description": "SELECT cart",
                    "start_timestamp": 1_700_000_000.05,
