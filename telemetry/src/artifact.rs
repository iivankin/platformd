...
pub async fn store_upload_chunk(
    ...
    if content_exists(store, "upload_chunk", &service.id, checksum).await? {
        return Ok(());
    }
    let now = Timestamp::now().to_string();
    let mut document = Document::base("upload_chunk", &service.id, &now, &now);
...
pub async fn missing_upload_chunks(
    ...
    for checksum in checksums {
        if !valid_sha1(checksum)
            || !content_exists(store, "upload_chunk", service_id, checksum).await?
        {
...
pub async fn assemble(
    ...
    for checksum in chunks {
        let chunk = load_content(store, "upload_chunk", &service.id, checksum).await?;
    ...
    let mut artifact = Document::base("artifact", &service.id, &now, &now);
    ...
    .await?
    {
        ...
        for (sequence, chunk) in assembled_chunks.into_iter().enumerate() {
            let mut document = Document::base("artifact_content_chunk", &service.id, &now, &now);
...
pub async fn load_artifact(store: &Store, service_id: &str, id: &str) -> Result<Vec<u8>> {
    load_content(store, "artifact_content_chunk", service_id, id).await
}
...
pub async fn source_bundles(
    ...
    load_artifacts_from_hits(store, service_id, hits).await
}
...
pub async fn debug_files(
    ...
    load_artifacts(store, service_id, all(terms)).await
}
...
async fn load_artifacts_from_hits(
    ...
    for hit in hits {
        ...
        artifacts.push(StoredArtifact {
            bytes: load_artifact(store, service_id, &id).await?,
            id,
            name: hit
                .get("object_name")
                .and_then(Value::as_str)
...
mod tests {
    ...
    async fn assembles_uploaded_chunks_without_changing_their_bytes() {
        ...
        let service = ServiceContext { id: "app".into() };
        ...
        for (checksum, chunk) in checksums.iter().zip(chunks) {
            store_upload_chunk(&store, &service, checksum, chunk)
                .await
                .unwrap();
        }
        ...
        assert_eq!(
            load_artifact(&store, &service.id, &artifact_checksum)
                .await
                .unwrap(),
            expected
        );
        ...
        assert!(
            proguard_mappings(&store, &service.id, Some("different"))
                .await
                .unwrap()
                .is_empty()
        );
        let mappings = proguard_mappings(&store, &service.id, Some("aabbccdd"))
            .await
            .unwrap();
