...
func (repository *auditRepository) AuditEvents(_ context.Context, query state.AuditQuery) (state.AuditPage, error) {
	...
	return state.AuditPage{Events: []state.AuditEvent{{
		ID: "event", ProjectID: "project", ActorKind: "token", ActorID: "token", Action: "server.exec",
...
func TestAuditAPIRequiresAccessAndPassesExactFilters(t *testing.T) {
	...
	response = httptest.NewRecorder()
	handler.ServeHTTP(response, projectRequest(http.MethodGet, "/api/v1/audit?projectId=project&actorKind=token&action=server.exec&result=succeeded&limit=25", ""))
	if response.Code != http.StatusOK || !strings.Contains(response.Body.String(), `"projectId":"project"`) || !strings.Contains(response.Body.String(), `"action":"server.exec"`) || repository.calls != 1 {
		t.Fatalf("audit response = %d/%s calls=%d", response.Code, response.Body, repository.calls)
	}
	if repository.query.ProjectID != "project" || repository.query.ActorKind != "token" || repository.query.Action != "server.exec" || repository.query.Result != "succeeded" || repository.query.Limit != 25 {
