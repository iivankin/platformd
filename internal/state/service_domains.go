...
func publicHostnameRoleExists(ctx context.Context, transaction *sql.Tx, hostname string) (bool, error) {
	return publicHostnameRoleExistsExcept(ctx, transaction, hostname, "")
}

func publicHostnameRoleExistsExcept(ctx context.Context, transaction *sql.Tx, hostname, exceptObjectStoreID string) (bool, error) {
	var exists int
	err := transaction.QueryRowContext(ctx, `
SELECT EXISTS(
