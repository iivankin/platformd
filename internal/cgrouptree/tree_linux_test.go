...
func TestTreeCreatesKillsAndRemovesOperationLeaf(t *testing.T) {
	mountRoot := t.TempDir()
	operationRoot := filepath.Join(mountRoot, "unit", operationsLeaf)
	if err := os.MkdirAll(operationRoot, 0o755); err != nil {
		t.Fatal(err)
	}
	tree := &Tree{mountRoot: mountRoot, operationPath: "/unit/" + operationsLeaf}
	leaf, err := tree.CreateOperationLeaf("exec-id")
	if err != nil {
		t.Fatal(err)
	}
	if leaf.FD() == 0 {
	...
	if err := os.WriteFile(filepath.Join(operationRoot, "exec-id", "cgroup.procs"), nil, 0o600); err != nil {
		t.Fatal(err)
	}
	if err := leaf.Kill(); err != nil {
		t.Fatal(err)
	}
	if value, err := os.ReadFile(filepath.Join(operationRoot, "exec-id", "cgroup.kill")); err != nil || string(value) != "1\n" {
	...
	leaf.file = nil
	if err := os.Remove(filepath.Join(operationRoot, "exec-id", "cgroup.kill")); err != nil {
		t.Fatal(err)
	}
	if err := os.Remove(filepath.Join(operationRoot, "exec-id", "cgroup.procs")); err != nil {
		t.Fatal(err)
	}
	if err := os.Remove(filepath.Join(operationRoot, "exec-id")); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(filepath.Join(operationRoot, "exec-id")); !os.IsNotExist(err) {
