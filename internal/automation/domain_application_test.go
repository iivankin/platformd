...
func (repository *domainRepositoryStub) AttachServiceDomain(_ context.Context, input state.AttachServiceDomainInput) (state.ServiceDomain, error) {
	...
	return state.ServiceDomain{Hostname: input.Hostname, ProjectID: input.ProjectID, ServiceID: input.ServiceID}, nil
}
...
func TestDomainApplicationEnforcesBoundaryAndCreatesTokenAuditInputs(t *testing.T) {
	...
	input := AttachDomainInput{ProjectID: bound, ServiceID: "service", Hostname: "app.example.com", Move: true}
	...
	if repository.attachCalls != 1 || repository.attach.ActorKind != "token" || repository.attach.ActorID != "admin" || !repository.attach.Move || result.RequestID == "" {
